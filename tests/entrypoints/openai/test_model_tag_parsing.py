...
def test_model_tag_positional_argument(api_server_parser):
    ...
    args = api_server_parser.parse_args([
        model_path,
        "--task", "generate",
        "--served-model-name", "opt-125m-server"
    ])
...
def test_async_engine_args_without_fix():
    ...
    with patch.object(AsyncEngineArgs, '__init__', return_value=None) as mock_init:
        ...
        with patch.object(AsyncEngineArgs, 'from_cli_args', 
